...
class ListDiningTables extends ListRecords
{
    ...
    {
        return [
            Action::make('exportExcel')->label('تنزيل الطاولات ورموز QR بصيغة Excel')
                ->icon('heroicon-o-arrow-down-tray')->color('success')
                ->url(fn (): string => route('admin.dining-tables.export')),
            Action::make('takeawayQr')->label('رمز QR للطلبات الخارجية')->url(route('qr.takeaway'))->openUrlInNewTab(),
